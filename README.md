...
AND / OR 条件で柔軟な検索を行うことができます。AND / OR は一覧画面で利用者が切り替えられます。
...
- テキスト：文字列（1行 / 複数行）・リッチエディター・リンク（like：部分一致）
- 選択肢：ドロップダウン / ラジオボタン / チェックボックス / 複数選択（in：選択肢の完全一致）
- 添付ファイル（like）
- 🔗 AND / OR 条件対応（一覧画面で利用者が切り替え可能）
...
### 対応フィールド
- フィールド種別	検索方法
- 文字列（1行 / 複数行）・リッチエディター・リンク	like（部分一致）
- ドロップダウン / ラジオボタン / チェックボックス / 複数選択	in（選択肢の完全一致）
- 添付ファイル	like
- サブテーブル内フィールド	対応

>※ 数値・日付・ユーザー選択・組織選択・グループ選択は対象外です（kintone 標準の絞り込みをご利用ください）。
...
複数の語をスペースで区切って入力したときの条件は、検索バーの「AND | OR」で利用者が切り替えられます。
- AND：すべての検索語を含むレコード
- OR：いずれかの検索語を含むレコード

プラグイン設定では、一覧画面を開いたときの AND / OR の初期値を管理者が指定します。利用者が選んだ条件は URL パラメータ（nrc_mode）として保持され、ページ送りや並べ替えの後も維持されます。

検索中は検索バーの下に「検索中: 東京 営業 / AND（すべて含む）」のように適用中の語と条件が表示され、「クリア」で解除できます。
一度に検索できる語数には上限（既定 5 語、設定で 1〜10）があり、超えた語や、選択肢に一致せず検索に使えなかった語は画面上で通知されます。

---

## 注意事項・制限
- ドロップダウン・ラジオボタン・チェックボックス・複数選択の検索は 選択肢の完全一致 のみ対象です
